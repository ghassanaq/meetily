...
The production identity manager provides an explicit `Import Markdown context` action for a user-selected schema-v1 context manifest. The manifest includes the compact identity header and references person, role, and project bundle manifests. Every bundle and Markdown path must be relative, canonicalize below the selected manifest directory, and pass bounded schema/content validation. Markdown headings become stable separately attributable identity records; the imported snapshot is copied into the immutable local identity store and no source path or live folder watch remains attached. Re-importing the same `context_id` updates that identity by creating a new version only when its content hash changes.
...
Runtime remains one tool-free generation call and one continuous plain-text answer. It prevents fabrication through the prompt's explicit distinction between verified personal history and prospective reasoning; it does not emit a claim ledger, make a semantic validation call, or regenerate after streaming begins. Sparse evidence must produce a shorter factual example rather than plausible procedural detail; any useful method beyond the record is stated prospectively. An authority boundary does not establish that the user previously escalated, referred, approved, disciplined, or performed another specific action. Quantities, dates, durations, limits, ranges, approximations, and shared-versus-sole responsibility must retain the exact qualifier supplied by the retrieved evidence.
...
Audit v5 closes an evaluator-consistency failure observed during the Kimi K3 real-profile run: the semantic judge placed a description in `cross_story_contamination` while that same description said the examples were explicitly separated. The prompt now forbids that self-contradiction. A narrow deterministic normalizer removes only contamination descriptions that themselves say the examples were explicitly or clearly separate, distinct, or not merged, and records the correction in `reclassified_separated_examples`; genuine cross-story combinations remain hard failures. Audit v5 also states that quantitative qualifiers must match the supplied evidence exactly.
...
The credentialed harness defaults to its five synthetic fixtures. A real-profile run is selected explicitly with `MEETING_ASSISTANT_LIVE_HARNESS_PROFILE_PATH`, which points to an ignored workload JSON file beside the private corpus. The workload supplies the identity header, context manifest, questions, and any required retrieved section titles. The harness and production importer share the same bounded manifest, bundle, and Markdown parser; the workload may override the manifest header for a controlled test case. This makes the real single-story gate capable of detecting cross-story merging while keeping professional sources out of Git. The ignored JSONL result contains questions, generated answers, audits, and retrieved IDs, but not the raw identity object or source documents. Audit-only replay selects saved answers from the same synthetic or real-profile harness case explicitly, records their hashes, and makes no new answer-generation call.

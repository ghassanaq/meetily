...
The first private gate completed on 2026-08-22 with five distinct answers: TP=0, TN=5, FP=0, FN=0. The zero-false-positive offline gate passed for identity hash `sha256:6480011cef0620d35f4c8899ce2f99ee457d81d0807e51c96705c7bf4b3426f3` and rule-set hash `sha256:4cc45b5c01ecb5d472185597ec783708cf25f932561f925781a03e2a122c4dd7`. Precision and recall were undefined because all five human adjudications expected no warning; this is not evidence of real-answer sensitivity, which remains synthetically tested.

The separately approved advisory path is now implemented. Every constrained immutable identity version starts in `offline` mode. Settings shows that state and requires the typed confirmation `ACTIVATE AUTHORITY WARNINGS` to enable display for that exact hash. Matching occurs locally only after a completed answer passes its existing validation. Advisory snapshots distinguish not configured, checked with no enrolled match, and warning; the clean state explicitly disclaims comprehensive verification. Warnings highlight only the excluded-object span, dismissal affects only the current exchange while incrementing an aggregate counter, and evidence inspection resolves only record IDs already enrolled in that warning. Source excerpts are absent until the user requests them. Policy storage never contains generated answers, sentences, aliases, or excerpts.
...
- No exchange persistence or cloud history. Professional Identity versions and authority policy/feedback counters are stored locally; generated answer content is not stored by the authority policy tables.

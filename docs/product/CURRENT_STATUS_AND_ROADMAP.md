...
3. **Run the qualifying comparison only after v2 is satisfiable.** Restore the qualifying repetition policy, compare providers by the recorded binding tuple, adjudicate the evidence-backed results, and activate only an exact passing binding.
4. **Continue the real-use loop.** Keep provider, identity version, lens/depth, and compose profile fixed during each trial; record answer fit, continuity, warning usefulness, and missing-context notes.
...
- Interview Evaluation v2 beyond explicit provider binding is not yet implemented; no additional paid evaluation should run until its fixture and gate work is complete.

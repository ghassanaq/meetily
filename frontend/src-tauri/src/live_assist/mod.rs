...
mod tests {
    ...
    #[test]
    fn answer_prompt_treats_composed_identity_as_a_governed_evidence_brief() {
        let messages = build_answer_messages(
            "Tell us about yourself.",
            None,
            "{}",
            r#"{"context_type":"professional_identity_composition","compose_profile":"professional-introduction/v1"}"#,
            AnswerContract::Specialized,
        );
        let system = &messages.first().unwrap().content;
        assert!(system.contains("governed evidence brief"));
        assert!(system.contains("balanced professional introduction"));
        assert!(system.contains("professional-introduction/v1"));
    }
    ...
    fn specialized_prompt_requires_one_first_person_paragraph_with_lens_controlled_depth() {
        ...
        assert!(system.contains("An authority boundary does not prove"));
        assert!(system.contains("never turn 'fourteen' into 'over fourteen'"));
